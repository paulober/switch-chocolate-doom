...
void M_SaveSelect(int choice)
{
    ...
    if (R_SUCCEEDED(rc) && SAVESTRINGSIZE > 0)
    {
        swkbdConfigMakePresetDefault(&kbd);
        swkbdConfigSetBlurBackground(&kbd, true);
        swkbdConfigSetHeaderText(&kbd, "Enter savegame name");
        swkbdConfigSetOkButtonText(&kbd, "Save");
        // TODO: check if special chars from other keyboards are also supported
        swkbdConfigSetType(&kbd, SwkbdType_QWERTY);
        swkbdConfigSetStringLenMax(&kbd, SAVESTRINGSIZE);
        swkbdConfigSetStringLenMin(&kbd, 2);
        if (strcmp(savegamestrings[choice], EMPTYSTRING))
        {
            swkbdConfigSetInitialText(&kbd, savegamestrings[choice]);
        }
        else
        {
        ...
        swkbdConfigSetKeySetDisableBitmask(
            &kbd, SwkbdKeyDisableBitmask_Backslash |
                      SwkbdKeyDisableBitmask_ForwardSlash |
                      SwkbdKeyDisableBitmask_Percent |
                      SwkbdKeyDisableBitmask_At);

        // Pull current savegame name into the keyboard
        char input[SAVESTRINGSIZE] = {0};
        rc = swkbdShow(&kbd, input, sizeof(input));

        if (R_SUCCEEDED(rc) && strlen(input) > 0)
        {
            if (input[0] == '\0' || strcmp(input, EMPTYSTRING))
            {
                goto closeSwkbd;
                // Cancelled
                return;
            ...
            M_StringCopy(saveOldString, savegamestrings[choice],
                         SAVESTRINGSIZE);
            M_StringCopy(savegamestrings[choice], input, SAVESTRINGSIZE);
            if (joypadSave)
            {
